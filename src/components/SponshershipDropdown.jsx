...
const DropdownSponshership = forwardRef((props, ref) => {
  const [departments, setDepartments] = useState([]);

  useEffect(() => {
    // Extracting departments from dummyData
    const uniqueDepartments = [...new Set(dummyData.map(item => item.sponsors))];
    setDepartments(uniqueDepartments);
  }, []); // Empty dependency array ensures this effect runs only once on initial render

  return (
    <div className="absolute ml-[770px] mt-[-30px] bg-[rgb(251,251,251)] rounded-[12px] max-w-[600px] flex flex-col z-50" ref={ref}>
...
export default DropdownSponshership;
